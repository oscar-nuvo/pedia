...
serve(async (req) => {
  ...
  try {
    ...
    // Built-in tools only - web search is handled natively by OpenAI
    // Custom function tools (calculate_pediatric_dosage, analyze_growth_chart) are disabled
    // because they require submitting results back to OpenAI which isn't implemented.
    // The model can still reason about dosages using its training data.
    const tools = [
      { type: "web_search" }
    ];
